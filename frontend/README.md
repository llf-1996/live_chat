...
| 变量名 | 说明 | 默认值 | 是否必需 |
|--------|------|--------|----------|
| `VITE_API_BASE_URL` | 后端 API 服务地址 | ❌ 无默认值 | **必需** |
| `VITE_WS_BASE_URL` | WebSocket 服务地址 | ❌ 无默认值 | **必需** |
| `VITE_PORT` | 前端开发服务器端口 | `5173` | 可选 |
...
5. **强制验证**：`VITE_API_BASE_URL` 和 `VITE_WS_BASE_URL` 未配置时会抛出异常

#### 验证配置

启动开发服务器后，打开浏览器控制台，应该能看到 WebSocket 连接成功。
